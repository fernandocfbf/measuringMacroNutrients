...
def apply_entropy(x, y, bias=1):
    ...
    if bias == 0:
        ...
        for indice in range(len(x)):
            ...
            #if the entropy is low, add the image to the training data
            if entropy < 11.999:
                x_shannon_entropy.append(x[indice])
                y_shannon_entropy.append(y[indice])

        return x_shannon_entropy, y_shannon_entropy
...
def backward_with_regularization(AL, Y, caches, lambd):
    """
    ...
    grads["dA" + str(L)], grads["dW" + str(L)], grads["db" + str(L)] = linear_activation_backward_with_regularization(dAL, current_cache, "sigmoid", lambd)
    
    for l in reversed(range(L-1)):
        # lth layer: (RELU -> LINEAR) gradients.
        current_cache = caches[l]
        dA_prev_temp, dW_temp, db_temp = linear_activation_backward_with_regularization(grads["dA" + str(l + 2)], current_cache, "relu", lambd)
